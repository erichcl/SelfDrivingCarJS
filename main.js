...
const traffic = [
  ...
  new Car(road.getLaneCenter(2), -615, 30, 50, 'DUMMY', 2),
];
animate();
...
function generateCars(N) {
  const cars = [];
  for (let i = 0; i < N; i++) {
    cars.push(new Car(road.getLaneCenter(1), 10, 30, 50, 'AI'));
  }
  return cars;
}
...
function animate(time) {
  ...
  bestCar = cars.find((c) => c.y == Math.min(...cars.map((mc) => mc.y)));
